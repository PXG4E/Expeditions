...
local Library = {
    ...
    TabTransitionInfo = TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
    TabSwipeOffset = 26,
    TabSwipeFrom = "bottom",

    WindowAnimationInfo = TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
    DropdownTransitionInfo = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
...
function Library:AddOutline(Frame: GuiObject)
    ...
    -- ShadowStroke removed to eliminate black blur effect
    return OutlineStroke, nil
end
...
do
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        function Toggle:Display()
            ...
            TweenService:Create(Checkbox, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                Size = UDim2.fromOffset(40, 40),
            }):Play()
            task.delay(0.12, function()
                if not Toggle.Destroyed then
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        -- DarkColor text stroke removed (caused black blur on slider values)
        ...
        if Info.AllowRightClickInput then
            ...
            -- DarkColor InputTextBox stroke removed
        end
        ...
        function Slider:Display()
            ...
            if CustomDisplayText then
                ...
                if Info.Compact then
                    local _fmtVal = string.format("%." .. tostring(Info.Rounding or 0) .. "f", Slider.Value)
                    DisplayLabel.Text =
                        string.format("%s: %s%s%s", Slider.Text, Slider.Prefix, _fmtVal, Slider.Suffix)
                elseif Info.HideMax then
                    local _fmtValH = string.format("%." .. tostring(Info.Rounding or 0) .. "f", Slider.Value)
                    DisplayLabel.Text = string.format("%s%s%s", Slider.Prefix, _fmtValH, Slider.Suffix)
                else
                    local _fmtValM = string.format("%." .. tostring(Info.Rounding or 0) .. "f", Slider.Value)
